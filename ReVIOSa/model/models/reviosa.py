...
class VideoLLaVASAMModel(LisaModel):
    ...
    def _add_special_tokens(self):
        ...
        self.seg_obj_token_idx = self.tokenizer("[SEG_OBJ]", add_special_tokens=False).input_ids[0]
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        gt_obj_masks = data.pop('gt_obj_masks', None)
        ...
        if gt_obj_masks is not None and gt_obj_masks.sum() > 0:
            seg_obj_valid = True
        else:
            seg_obj_valid = False
        ...
        if seg_obj_valid:
            pred_obj_embeddings = hidden_states[seg_obj_token_mask] + _zero
        else:
            pred_obj_embeddings = hidden_states[:, :5].flatten(0, 1) + _zero
        ...
        pred_embeddings_list_video, success = self.generate_video_pred_embeddings(
...
class ReVIOSaModel(VideoLLaVASAMModel):
    def __init__(self,
                 mllm,
                 tokenizer,
                 grounding_encoder,
        ...
        super(ReVIOSaModel, self).__init__(
            mllm=mllm,
            tokenizer=tokenizer,
        ...
        self.image_token_size = int(np.sqrt(self.patch_token))
        self.img_context_token = '<IMG_CONTEXT>'
        self.img_start_token = '<img>'
        self.img_end_token = '</img>'
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        g_pixel_values = data.pop('g_pixel_values', None)
        gt_masks = data.pop('masks', None)
        gt_obj_masks = data.pop('obj_masks', None)
        frames_per_batch = data.pop('frames_per_batch', None)
        input_ids = data['input_ids']
        batch_size, n_frames, n_exps = len(data['input_ids']), data['pixel_values'].shape[1], int(gt_masks[0].shape[0] / data['pixel_values'].shape[1])
        if self.fast_pool:
            output = self.mllm(data, data_samples, mode, fast_token_idx=self.fast_token_idx)
        else:
            output = self.mllm(data, data_samples, mode)
        ...
        output:
            ...
            hidden_states: list[tensor[batch_size, seq_len, hidden_dim]]
            (optional) attentions : n_layers * (batch_size * tensor[n_heads, seq_len, seq_len])
        '''
        if gt_masks is None:
            # require zero seg datas
            seg_valid = False
        ...
        if gt_obj_masks is None:
            seg_obj_valid = False
            gt_obj_masks = []
            for bs_idx in range(batch_size):
                gt_obj_masks.append(torch.zeros((n_frames * n_exps, gt_masks[bs_idx].shape[1], gt_masks[bs_idx].shape[2])).to(gt_masks[0].device))
        else:
            seg_obj_valid = True
        
        unidirectional_list = []
        if 'caption_type' in data:
            for caption_types in data['caption_type']:
                unidirectional_list.append([])
                for caption_idx, caption_type in enumerate(caption_types):
                    if caption_type == 'unidirectional':
                        unidirectional_list[-1].append(caption_idx)
        else:
            for bs_idx in range(batch_size):
                unidirectional_list.append([])
        ...
        seg_token_mask = input_ids == self.seg_token_idx
        seg_obj_token_mask = input_ids == self.seg_obj_token_idx
        ...
        if seg_valid:
            ...
            pred_embeddings = hidden_states[:, :5].flatten(0, 1) + _zero  # dummy
        
        if seg_obj_valid:
            pred_obj_embeddings = hidden_states[seg_obj_token_mask] + _zero
        else:
            pred_obj_embeddings = hidden_states[:, :5].flatten(0, 1) + _zero  # dummy
        
        seg_token_counts = seg_token_mask.int().sum(-1)
        seg_obj_token_counts = seg_obj_token_mask.int().sum(-1)
        
        if not seg_valid:
            seg_token_counts += 5
        if not seg_obj_valid:
            seg_obj_token_counts += 5

        pred_embeddings_list_ = torch.split(pred_embeddings, seg_token_counts.tolist(), dim=0)
        pred_obj_embeddings_list_ = torch.split(pred_obj_embeddings, seg_obj_token_counts.tolist(), dim=0)
        ...
        pred_obj_embeddings_list = []
        for uni_list, item in zip(unidirectional_list, pred_obj_embeddings_list_):
            items = torch.zeros_like(pred_embeddings_list[0], dtype=pred_embeddings_list[0].dtype, device=pred_embeddings_list[0].device)
            for idx, item_idx in enumerate(uni_list): 
                items[item_idx] += item[idx]
            pred_obj_embeddings_list.append(items)
        pred_obj_embeddings_list_video, _ = self.generate_video_pred_embeddings(
            pred_obj_embeddings_list, frames_per_batch)
        
        gt_obj_masks_video = self.process_video_gt_masks(gt_obj_masks, frames_per_batch)
        pred_obj_embeddings_list_video, gt_obj_masks_video = self.check_obj_number(
            pred_obj_embeddings_list_video, gt_obj_masks_video
        )
        ...
        language_embeddings_obj = torch.cat(pred_obj_embeddings_list_video, dim=0)[:, None]
        pred_obj_masks = self.grounding_encoder.inject_language_embd(sam_states, language_embeddings_obj, nf_nobj=(num_frames, num_objs))
        
        gt_obj_masks = [F.interpolate(gt_obj_mask.unsqueeze(0), size=pred_masks[0].shape[-2:], mode='nearest').squeeze(0) for gt_obj_mask in gt_obj_masks_video]
        gt_obj_masks = torch.cat(gt_obj_masks, dim=0)
        pred_obj_masks = pred_obj_masks.flatten(0, 1)

        uni_mask = torch.zeros((batch_size, n_frames, n_exps), dtype=pred_masks.dtype, device=pred_masks.device)
        for batch_idx, uni_list in enumerate(unidirectional_list):
            for exp_idx in uni_list:
                uni_mask[batch_idx, :, exp_idx] = 1.0
        valid_mask = uni_mask.reshape(-1, 1, 1)
        invalid_mask = (1.0 - valid_mask)
        pred_obj_masks = pred_obj_masks * valid_mask + pred_obj_masks.detach() * invalid_mask
        pred_obj_masks_ = pred_obj_masks * valid_mask
        gt_obj_masks_   = gt_obj_masks   * valid_mask
        num_valid = (valid_mask.sum() * pred_obj_masks.shape[-1] * pred_obj_masks.shape[-2]).clamp_min(1.0)

        # loss
        bs = len(pred_masks)
        loss_mask, loss_dice, loss_attn_mask = 0, 0, 0
        if len(pred_masks) != len(gt_masks):
            # drop this data
            print(f"Pred mask shape {pred_masks.shape} is not equal to gt_mask shape {gt_masks.shape} !!!")
            min_num = min(len(pred_masks), len(gt_masks))
        ...
        if self.loss_sample_points:
            ...
            if valid_mask.sum() < 0.5:
                sam_loss_mask_obj = pred_obj_masks.mean() * 0.0
                sam_loss_dice_obj = pred_obj_masks.mean() * 0.0
            else:
                sampled_pred_obj_mask, sampled_gt_obj_mask = self.sample_points(pred_obj_masks_, gt_obj_masks_)
                num_valid_points = (sampled_pred_obj_mask.numel() // sampled_pred_obj_mask.shape[-1]) * sampled_pred_obj_mask.shape[-1]
                sam_loss_dice_obj = self.loss_dice(
                    sampled_pred_obj_mask,
                    sampled_gt_obj_mask,
                    avg_factor=(len(gt_masks) + 1e-4))
                sam_loss_mask_obj = self.loss_mask(
                    sampled_pred_obj_mask.reshape(-1),
                    sampled_gt_obj_mask.reshape(-1),
                    avg_factor=(num_valid_points + 1e-4)
                )
        else:
            # ACTOR
            sam_loss_mask = self.loss_mask(pred_masks, gt_masks)
            sam_loss_dice = self.loss_dice(pred_masks, gt_masks)
            # TARGET
            sam_loss_mask_obj = self.loss_mask(pred_obj_masks_, gt_obj_masks_, avg_factor=(num_valid + 1e-4))
            sam_loss_dice_obj = self.loss_dice(pred_obj_masks_, gt_obj_masks_, avg_factor=(valid_mask.sum() + 1e-4))
        loss_mask += sam_loss_mask
        loss_dice += sam_loss_dice
        loss_mask += sam_loss_mask_obj
        loss_dice += sam_loss_dice_obj

        # RESIZE GT MASKLET INTO 16X16 (ATTENTION MAP SIZE)
        gt_masks_resized = F.interpolate(gt_masks.unsqueeze(1).float(), size=(self.image_token_size, self.image_token_size), mode='nearest')
        gt_masks_resized = gt_masks_resized.view(batch_size, n_exps, n_frames * self.image_token_size * self.image_token_size)
        gt_obj_masks_resized = F.interpolate(gt_obj_masks.unsqueeze(1).float(), size=(self.image_token_size, self.image_token_size), mode='nearest')
        gt_obj_masks_resized = gt_obj_masks_resized.view(batch_size, n_exps, n_frames * self.image_token_size * self.image_token_size)

        # ATTENTION MAP MASK LOSS
        attn_mask_losses = []
        for layer_head_idx, attn in enumerate(output.attentions):
            vision_attn_tensor, vision_obj_attn_tensor = [], []
            for idx in range(batch_size):
                n_seg_tokens = int(seg_token_mask[idx].sum(-1))

                # [SEG] attention maps
                vision_attn_map = attn[idx][seg_token_mask[idx]].reshape(1, n_seg_tokens, -1)

                # [SEG_OBJ] attention maps
                n_obj_seg_tokens = int(seg_obj_token_mask[idx].sum(-1))
                if n_obj_seg_tokens != 0:
                    vision_obj_attn_map = attn[idx][seg_obj_token_mask[idx]].reshape(1, n_obj_seg_tokens, -1)

                seg_indices = torch.nonzero(seg_token_mask[idx], as_tuple=True)[0]
                obj_indices = torch.nonzero(seg_obj_token_mask[idx], as_tuple=True)[0]
                obj_pos = torch.bucketize(obj_indices, seg_indices) - 1

                # Extract vision token indices
                img_start_token_pos_list = [i for i, token_id in enumerate(input_ids[idx]) if token_id == self.img_start_token_id]
                img_end_token_pos_list = [i for i, token_id in enumerate(input_ids[idx]) if token_id == self.img_end_token_id]
                img_context_token_ranges = [(start + 1, end) for start, end in zip(img_start_token_pos_list, img_end_token_pos_list)]
                vision_token_indices = [i for start, end in img_context_token_ranges for i in range(start, end)]
                vision_token_indices_tensor = torch.tensor(vision_token_indices, device=attn.device)

                assert len(vision_token_indices_tensor) == self.patch_token * n_frames, \
                    f"Expected {self.patch_token * n_frames} vision tokens, got {len(vision_token_indices_tensor)}"

                vision_attn_tensor = vision_attn_map[:, :, vision_token_indices_tensor]
                if n_obj_seg_tokens != 0:
                    vision_obj_attn_tensor = vision_obj_attn_map[:, :, vision_token_indices_tensor]
                
                # Compute loss
                act_loss = F.binary_cross_entropy(vision_attn_tensor.to(dtype=gt_masks_resized.dtype), gt_masks_resized[idx].unsqueeze(0))
                if n_obj_seg_tokens != 0:
                    tar_loss = F.binary_cross_entropy(vision_obj_attn_tensor.to(dtype=gt_obj_masks_resized.dtype), gt_obj_masks_resized[idx][obj_pos].unsqueeze(0))
                else:
                    tar_loss = 0.0
                attn_mask_losses.append(act_loss + tar_loss)

        # Sum all losses across layers/heads
        loss_attn_mask = sum(attn_mask_losses) / len(attn_mask_losses)
        ...
        loss_mask = loss_mask * _scale
        loss_dice = loss_dice * _scale
        loss_attn_mask = loss_attn_mask * _scale
        
        loss_dict = {
            'loss_mask': loss_mask,
            'loss_dice': loss_dice,
            'llm_loss': output.loss,
            'attn_mask_loss': loss_attn_mask,
        }
        return loss_dict

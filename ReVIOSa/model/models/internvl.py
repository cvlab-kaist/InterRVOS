...
class InternVL_Slowfast(InternVL_V1_5):
    ...
    def __init__(self,
        ...
        config.llm_config.output_attentions = False if use_flash_attn else True

        self.attention_key_to_save = attention_key_to_save
    ...
    def forward(self, data, data_samples=None, mode='loss', fast_token_idx=None):
        ...
        if 'sampled_indexes' in data.keys():
            sampled_indexes = data['sampled_indexes']
        else:
            sampled_indexes = None
        if 'expressions' in data.keys():
        ...
        outputs = self._llm_forward(
            ...
            labels=labels,
            use_cache=use_cache,
            output_attentions=self.model.config.llm_config.output_attentions,
            ...
            attention_key_to_save=self.attention_key_to_save,
        )
    ...
    def _llm_forward(
        ...
        if fast_pixel_values is not None:
            ...
            fast_vit_embeds = vit_embeds[:n_fast_images]
            ...
            # pooling
            fast_vit_embeds = fast_vit_embeds.permute(0, 3, 1, 2)
            fast_vit_embeds = self.fast_pool(fast_vit_embeds).flatten(2)
        ...
        outputs = self.model.language_model(
            ...
            attention_key_to_save=attention_key_to_save,
        )

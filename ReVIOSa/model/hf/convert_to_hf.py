...
def main():
    ...
    from model.hf.models.configuration_reviosa_chat import ReVIOSaChatConfig
    from model.hf.models.modeling_reviosa_chat import ReVIOSaChatModel

    internvl_config = ReVIOSaChatConfig.from_pretrained(cfg.path)
    config_dict = internvl_config.to_dict()
    config_dict['auto_map'] = \
        {'AutoConfig': 'configuration_reviosa_chat.ReVIOSaChatConfig',
         'AutoModel': 'modeling_reviosa_chat.ReVIOSaChatModel',
         'AutoModelForCausalLM': 'modeling_reviosa_chat.ReVIOSaChatModel'}

    config_dict["llm_config"]["vocab_size"] = len(model.tokenizer)
    config_dict["template"] = cfg.template
    reviosa_hf_config = ReVIOSaChatConfig(
        **config_dict
    )
    hf_reviosa_model = ReVIOSaChatModel(
        reviosa_hf_config, vision_model=model.mllm.model.vision_model,
        language_model=model.mllm.model.language_model,
    )
    hf_reviosa_model.load_state_dict(all_state_dict_new)

    hf_reviosa_model.save_pretrained(args.save_path)
    model.tokenizer.save_pretrained(args.save_path)
    print(f"Save the hf model into {args.save_path}")

    # copy the files
    os.system(f"cp -pr ./model/hf/models/* {args.save_path}")

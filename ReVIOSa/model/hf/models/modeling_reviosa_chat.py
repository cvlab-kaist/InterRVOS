...
from .configuration_reviosa_chat import ReVIOSaChatConfig
...
class ReVIOSaChatModel(PreTrainedModel):
    config_class = ReVIOSaChatConfig
    ...
    def __init__(self, config: ReVIOSaChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
    ...
    def preparing_for_generation(self, tokenizer, max_new_tokens=2048, torch_dtype=torch.bfloat16):
        ...
        self.seg_obj_token_idx = tokenizer.convert_tokens_to_ids('[SEG_OBJ]')
        return
    ...
    def predict_forward(
        ...
        if image is None and video is None and '<image>' not in past_text:
            return {'prediction': predict, 'prediction_masks': ret_masks_act, }
        ...
        # ACTOR
        seg_act_hidden_states = get_seg_hidden_states(
            last_hidden_states, generate_output.sequences[0][:-1],
            seg_id=self.seg_token_idx
        )
        all_seg_act_hidden_states = self.text_hidden_fcs(seg_act_hidden_states)
        for seg_act_hidden_states in all_seg_act_hidden_states:
            seg_act_hidden_states = seg_act_hidden_states.unsqueeze(0)
            g_pixel_values = input_dict['g_pixel_values']
            sam_states = self.grounding_encoder.get_sam2_embeddings(g_pixel_values)
            pred_masks = self.grounding_encoder.language_embd_inference(sam_states, [seg_act_hidden_states] * num_frames)
            w, h = ori_image_size
            masks = F.interpolate(pred_masks, size=(h, w), mode='bilinear', align_corners=False)
            masks = masks[:, 0]
            masks = masks.sigmoid() > 0.5
            masks = masks.cpu().numpy()
            ret_masks_act.append(masks)

        # TARGET
        if '[SEG_OBJ]' in predict:
            seg_tar_hidden_states = get_seg_hidden_states(
                last_hidden_states, generate_output.sequences[0][:-1],
                seg_id=self.seg_obj_token_idx
            )
            all_seg_tar_hidden_states = self.text_hidden_fcs(seg_tar_hidden_states)
            for seg_tar_hidden_states in all_seg_tar_hidden_states:
                seg_tar_hidden_states = seg_tar_hidden_states.unsqueeze(0)
                g_pixel_values = input_dict['g_pixel_values']
                sam_states = self.grounding_encoder.get_sam2_embeddings(g_pixel_values)
                pred_masks = self.grounding_encoder.language_embd_inference(sam_states, [seg_tar_hidden_states] * num_frames)
                w, h = ori_image_size
                masks = F.interpolate(pred_masks, size=(h, w), mode='bilinear', align_corners=False)
                masks = masks[:, 0]
                masks = masks.sigmoid() > 0.5
                masks = masks.cpu().numpy()
                ret_masks_tar.append(masks)
        else:
            ret_masks_tar = []

        return {
            'prediction': predict,
            'prediction_masks_act': ret_masks_act,
            'prediction_masks_tar': ret_masks_tar,
            }

...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_obj_mask = any(inst.get('obj_masks') is not None for inst in instances)
    ...
    has_caption_type = any(inst.get('caption_type') is not None for inst in instances)
    has_sampled_indexes = any(inst.get('sampled_indexes') is not None for inst in instances)
    has_expressions = any(inst.get('expressions') is not None for inst in instances)
    ...
    if has_sampled_indexes:
        sampled_indexess = []
    else:
        sampled_indexess = None
    ...
    for example in instances:
        if has_caption_type:
            caption_types.append(example['caption_type'])
        if has_sampled_indexes:
            sampled_indexess.append(example['sampled_indexes'])
        if has_expressions:
            expressions.append(example['expressions'])
        ...
        if has_obj_mask:
            if 'obj_masks' in example.keys() and example['obj_masks'] is not None:
                if isinstance(example['obj_masks'], list):
                    if isinstance(example['obj_masks'][0], np.ndarray):
                        _masks = np.stack(example['obj_masks'], axis=0)
                        _masks = torch.from_numpy(_masks)
                        object_obj_masks.append(_masks)
                    else:
                        object_obj_masks.append(torch.stack(example['obj_masks'], dim=0))
                else:
                    object_obj_masks.append(example['obj_masks'])
        if has_bboxes:
    ...
    if has_obj_mask:
        data_dict['obj_masks'] = object_obj_masks

    if has_caption_type:
        data_dict['caption_type'] = caption_types

    if has_sampled_indexes:
        data_dict['sampled_indexes'] = sampled_indexess

    if has_expressions:
        data_dict['expressions'] = expressions

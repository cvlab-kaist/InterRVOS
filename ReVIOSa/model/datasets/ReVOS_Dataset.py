...
class VideoReVOSDataset(Dataset):
    ...
    def prepare_text(self, n_frames, expressions, num_image_tokens=256, n_fast_images=50):
        ...
        for i, exp in enumerate(expressions):
            ...
            if '?' in exp:
                ...
                questions.append(question_template.format(expression=exp.lower()))
    ...
    def __getitem__(self, index):
        ...
        '''
        data_dict:
            images: list[str], 비디오 프레임들의 경로
            video_masks: list[list[list[dict]]], 비디오 프레임들의 마스크
            conversation: list[dict],
            fast_images: None
            fast_video_masks: None
            pixel_values: tensor[T, C, H, W], 비디오 프레임들의 pixel values
            g_pixel_values: list[tensor[C, H, W]], 비디오 프레임들의 pixel values
            masks: tensor[T x E, H, W], 비디오 프레임들의 마스크
        '''
